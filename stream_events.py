...
def event_stream(total=1000, seed=42):
    ...
    for n in range(1, total + 1):
        ...
        # Matched 9 weights to 9 actions
        ...
        # Anomaly 1: Exfiltration
        if n in range(int(total * 0.20), int(total * 0.20) + 20):
            u = users[99]  # Fixed index out-of-bounds (9999 -> 99)
        ...
        # Anomaly 2: Compromised Account
            ...
            bytes_out = random.randint(10_000, 100_000) # Fixed min/max order swap
            src = '45.77.21.13'
            status = random.choice(['failed','success'])
            risk = 85
            label = 'compromised_account'

        # Anomaly 3: Privilege Abuse
        ...
        yield {
            ...
            'status': status,
            'bytes_out': bytes_out,
            'records_accessed': max(0, int(bytes_out / random.randint(10, 100))), # Fixed 0 division
            'latency_ms': max(1, int(random.gauss(120, 30))), # Fixed gauss arguments
